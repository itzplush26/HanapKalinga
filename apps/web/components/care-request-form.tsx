...
const emptyValues: CareRequestFormValues = {
  title: "",
  patientCondition: "",
  careType: "per_shift",
...
interface CareRequestFormProps {
  ...
  initialValues?: CareRequestFormValues;
}

export function CareRequestForm({ mode, careRequestId, initialValues }: CareRequestFormProps) {
  ...
  async function handleSubmit(values: CareRequestFormValues) {
    ...
    const url = isEdit ? `/api/care-requests/${careRequestId}` : "/api/care-requests";
      ...
      setStatus(
        payload.error ??
          (isEdit
            ...
            : "Could not post your care request. Please try again.")
      );
      return;
    }

    const id = payload.careRequestId ?? careRequestId;
    if (id) {
      router.push(
        `/dashboard/family/care-requests/${id}${isEdit ? "?updated=1" : "?posted=1"}`
      );
    }
  }
  ...
  return (
    <>
      <PageHeader title={isEdit ? "Edit care request" : "Post a care request"} />
      <main className="px-5 py-6">
        <div className="mx-auto flex max-w-md flex-col gap-4">
          {isEdit ? (
            <p className="text-sm text-text-secondary">
              Update your listing while it is still open. Nurses who already applied will see your
              latest details.
            </p>
          ) : null}
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            ...
            >
              {isEdit ? "Save changes" : "Post care request"}
            </LoadingButton>
          </form>
        </div>
      </main>
